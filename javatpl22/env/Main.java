...
public class Main {
  static GetMonsters pz = new GetMonsters();
  ...
  public static void main(String[] args) {
    ...
    while (true) {
      try {
        Thread.sleep(1000);
        if (pz.getBalls() + pz.getSuperBalls() + pz.getHyperBalls() > 0) {
          pz.move();
          System.out.println("手持ちのボールは" + (pz.getBalls() + pz.getSuperBalls() + pz.getHyperBalls()) + "個");
    ...
    for (int i = 0; i < pz.getUserMonster().length; i++) {
      if (pz.getUserMonster()[i] != null) {
        if (pz.getMonsterTitle()[i] != null) {
          System.out.println(pz.getMonsterTitle()[i] + pz.getUserMonster()[i] + "を捕まえた．");
        } else {
